...
<?php
if (isset($_POST["year"])){
    header('Location: customer.php?year=' .$_POST["year"]);
}
?>  
...
<body><?php $year=$_SESSION['taon']; ?>
	<header>
        ...
        <nav>
            <ul>
                ...
                <?php echo "<li><div class='where'><a href='customer.php?year=$year' >SALES</a></li></div>"?>
    ...
    <section class="banner">
        <h1>Customers in branch <?php echo $_SESSION['id'];?></h1>
    </section>
    <div class="center">
    ...
    $query = "select * from customer natural join branch natural join buys_at natural join automobile natural join automobile_specs where branch_id = $sessid and YEAR(purchase_date)=$date order  by cus_ID ";
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        while($row = mysqli_fetch_assoc($result)){
            ...
            $dy = $date_arr[2];
            $qty = $row['quantity'];
            echo "<td>".$row['automobile_modelname']."</td><td>".$row['quantity']."</td><td>₱".$row['automobile_price']."</td><td>₱".$row['paid']."</td><td>".$row['purchase_date']."</td><td><button onclick=\"location.href='./edit/editcustomers.php?id=$emid&branch=$branchid&car=$carid&day=$dy&month=$mth&yr=$yr&qty=$qty'\">Edit</button></td> <td><button onclick=\"ConfirmDelete($emid,$branchid,$carid,$yr,$mth,$dy,$qty);\">Delete</button></td></tr>";
        }
    }
        echo "</table>";?><br> 
        </div>
        <div class="bit">
            <button class="add" onclick="location.href='./add/addcustomer.php'">Add New Customer</button>
		</div>
    </div>
<?php } else{ header("Location: ../login/login.php");}?>

<script>
    function ConfirmDelete($emid,$branchid,$carid,$yr,$mth,$dy,$qty){
        if(confirm("Are you sure you want to delete?")){
            ...
            var l = $qty;
            alert("Deleted");
            location.href="./delete/deletecustomer.php?id=" + x + "&branch=" + y + "&car=" + z + "&day=" + k + "&month=" + h+ "&yr=" + w +"&qty=" + l;
