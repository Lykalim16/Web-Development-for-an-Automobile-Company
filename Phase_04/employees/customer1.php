...
<body><?php $year=$_SESSION['taon']; ?>
    <header>
        ...
        <nav>
            <ul>
                ...
                <li><div class='where'><a href="customer1.php">CUSTOMER</a></li></div>
    ...
    <section class="banner">
        <h1>Branch <?php echo $_SESSION['id'];?> Customers</h1>
    </section>
    ...
    $query = "select distinct cus_ID,cus_fname,cus_mname,cus_lname,cus_streetNum,cus_baranggay,cus_city,cus_province,cus_zipCode from customer natural join customer_contact natural join buys_at where branch_id = $sessid order  by cus_ID ";
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        while($row = mysqli_fetch_assoc($result)){
            ...
            $cusid=$row['cus_ID'];
            echo "<td><button onclick=\"location.href='./edit/editcustomerdeets.php?id=" .$row['cus_ID']."'\">Edit</button></td>

            <td><button onclick=\"ConfirmDelete($cusid);\">Delete</button></td><td><button onclick=\"location.href='./edit/purchases.php?id=" .$row['cus_ID']."'\">Purchases</button></td></tr>";
        }
    }
        echo "</table>";
        ?>
...
<script>
    function ConfirmDelete($cusid){
        if(confirm("Are you sure you want to delete?")){
            var x = $cusid;
            alert("Deleted");
            location.href="./delete/deletecustomerdeets.php?id=" + x ;
