...
<?php $year=$_SESSION['taon']; ?>
        ...
        <nav>
            <ul>
               <li><a href="adminapproval.php">REQUESTS</a></li>
                ...
                <?php echo "<li><div class='where'><a href='customer.php?year=$year' >CUSTOMER</a></li></div>"?>
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        $query="select distinct cus_ID,cus_fname,cus_mname,cus_lname,cus_streetNum,cus_baranggay,cus_city,cus_province,cus_zipCode from customer natural join buys_at where branch_id =". $row["branch_ID"];
            $results = mysqli_query($conn,$query);
            if(mysqli_num_rows($results) > 0){

                echo "<h1 style='font-size:30px'>Branch ". $row['branch_ID']."</h1>";
                echo "<table>
                <tr>
                    <th>Customer ID</th>
                    <th> First Name</th>
                ...
                while($rows = mysqli_fetch_assoc($results)){
                    echo "<tr><td>".$rows['cus_ID']."</td>
                    <td>".$rows['cus_fname']."</td>
                    <td>".$rows['cus_mname']."</td>
                    <td>".$rows['cus_lname']."</td>
                    <td>".$rows['cus_streetNum']."</td>
                    <td>".$rows['cus_baranggay']."</td>
                    <td>".$rows['cus_city']."</td>
                    <td>".$rows['cus_province']."</td>
                    <td>".$rows['cus_zipCode']."</td>";
                
                    $query = "select * from customer_contact where cus_ID = ".$rows["cus_ID"];
                    ...
                    $emid=$rows['cus_ID'];
                     echo "
                     <td><button onclick=\"location.href='./edit/editcustomer.php?id=" .$rows['cus_ID']."'\">Edit</button></td>

                     <td><button onclick=\"ConfirmDelete($emid);\">Delete</button></td><td><button onclick=\"location.href='./edit/purchases.php?id=" .$rows['cus_ID']."'\">Purchases</button></td></tr>";
...
?> <br>
    ...
    function ConfirmDelete($emid){
        if(confirm("Are you sure you want to delete?")){
            var x = $emid;
            alert("Deleted");
            location.href="./delete/deletecustomer.php?id=" + x;
