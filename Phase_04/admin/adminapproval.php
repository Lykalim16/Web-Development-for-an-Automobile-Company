...
<body> <?php if (isset($_SESSION["id"])){ 
    ...
    <header>
        ...
        <nav>
            <ul>
				<li><div class="where"><a href="adminapproval.php">REQUESTS</a></li></div>
	...
	$query = "select * from temp_user order by id";
	$result = mysqli_query($conn,$query);
	if(mysqli_num_rows($result) > 0){
		echo "<table>
	    <tr>
	    	<th colspan=\"10\"> PENDING ACCOUNT APPROVALS</th>
	    </tr>
		<tr>
			<td> ID</th>
			<td> First Name</th>
			<td> Middle Initial</th>
			<td> Last Name</th>
		...
		while($row = mysqli_fetch_assoc($result)){
			...
			<td>".$row['email']."</td>
			<td>".$row['branchid']."</td>
			<td><button type='submit' name='submit' onclick=\"location.href='approveuser.php?id=".$row['id']."'\">Approve</button></td>
		<td><button onclick=\"location.href='deleteuser.php?id=".$row['id']."'\">Deny</button></td></tr>";
		}
		echo "</table>";
	}
	else{
		...
		<tr>
			<td> THERE ARE NO PENDING APPROVALS </th>
			</tr>";
		echo "</table>";
	}
?>
