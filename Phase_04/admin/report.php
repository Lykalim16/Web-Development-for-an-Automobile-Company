...
<body><?php if (isset($_SESSION["id"])){ 
        ...
        <nav>
            <ul>
               <li><a href="adminapproval.php">REQUESTS</a></li>
                ...
                <?php echo "<li><div class='where'><a href='report.php?year=$year' >REPORT </a></li></div>"?>
    ...
    <div class="banner">
        <h1>Branch Sale's History</h1>
    </div>
    <div class="center">
        <div class="employeeform">
        <div class="yearchoice">
                <form action="./report.php" method="post">
          <h1>Branch Sales History For <select name="year" onchange="this.form.submit()">
            <?php 
        ...
        if(!$conn) die(mysqli_error($conn));if(!$conn) die(mysqli_error($conn));
        $query="select * from branch";
        $results = mysqli_query($conn,$query);
        if(mysqli_num_rows($results) > 0){
            while($rows = mysqli_fetch_assoc($results)){
                $brid=$rows['branch_ID'];
                $query = "select * from  buys_at natural join customer natural join branch natural join automobile natural join automobile_specs where Year(purchase_date) =$date and branch_id =$brid order by purchase_date";
                $result = mysqli_query($conn,$query);
                if(mysqli_num_rows($result) > 0){

                    echo "<h2 style='font-size:20px';>Branch ". $rows['branch_ID']."</h2>";
                    echo "<table>
                    <thead>
                        <tr>
        ...
        if(mysqli_num_rows($results) > 0){
            while($rows = mysqli_fetch_assoc($results)){
                $brid=$rows['branch_ID'];
                $query = "select * from automobile natural join automobile_specs natural join tracks natural join has_inventory natural join branch where branch_id =$brid and in_year=$date";
                $result = mysqli_query($conn,$query);
                $pesosign = "₱";
                if(mysqli_num_rows($result) > 0){
