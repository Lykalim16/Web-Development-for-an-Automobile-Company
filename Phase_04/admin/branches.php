...
<?php if (isset($_SESSION["id"])){ 
    ...
    <div class="center">
        ...
        <?php
				...
				if(mysqli_num_rows($result) > 0){
					echo "<table>
					<tr><th>Branch ID</th><th>Name</th>
					<th>Branch Street Number</th><th>Branch Baranggay</th><th>Branch Zip Code</th>
                    <th colspan='2'> Contact Num</th><th>Edit</th><th>Delete</th></tr>";
					while($row = mysqli_fetch_assoc($result)){
                        ...
                        $querys = "select * from branch_contact where branch_ID = ". $row["branch_ID"];
                        $results = mysqli_query($conn,$querys);
                        if (mysqli_num_rows($results) ==1){
                            $rows = mysqli_fetch_assoc($results);
                            echo "<td>".$rows['branch_cnum']."</td>
                            <td></td>";
                        }
                        ...
                        echo "<td><button onclick=\"location.href='./edit/editbranches.php?id=".$row['branch_ID']."'\">Edit</button></td>
                        <td><button onclick=\"ConfirmDelete($branchid);\">Delete</button></td></tr>";
					}
					echo "</table>";
				}
			?>
            <div class="bit">
            <button class="add" onclick="location.href='./add/addbranch.php'">Add New Branch</button>
            </div>
        </div>
    </div>
    ...
    function ConfirmDelete($branchid){
        if(confirm("Are you sure you want to delete?")){
            var x = $branchid;
            alert("Deleted");
            location.href="./delete/deletebranch.php?id="+x;
